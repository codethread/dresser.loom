...
- **TASK-Dresser-001.MI3:** Tests in `test/skein/spools/dresser_test.clj` (extend the existing ns): registry completeness (every IC8-referenced key present), parameterized templates render with a sample name and contain no leftover placeholder, unknown-name and missing-param lookups throw with data, and no template line exceeds column 180.
